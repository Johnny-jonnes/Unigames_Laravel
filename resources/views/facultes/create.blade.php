<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-[20px] font-bold text-text-primary tracking-tight">Ajouter une Institution</h1>
                <p class="text-[13px] font-medium text-text-muted mt-0.5">Enregistrez une nouvelle faculté dans le système.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <form action="{{ route('facultes.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nom" id="nom" class="enterprise-input {{ $errors->has('nom') ? 'border-danger' : '' }}" value="{{ old('nom') }}" placeholder="Ex: Faculté des Sciences" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="universite" id="universite" class="enterprise-input {{ $errors->has('universite') ? 'border-danger' : '' }}" value="{{ old('universite') }}" placeholder="Ex: Université Gamal Abdel Nasser" required>
                    ...
                    <div>
                        ...
                        <select name="edition_id" id="edition_id" class="enterprise-input px-4 appearance-none {{ $errors->has('edition_id') ? 'border-danger' : '' }}" required>
                            <option value="" disabled selected>Sélectionner une édition ▾</option>
            ...
            <div class="flex items-center justify-end gap-3 bg-white p-4 rounded-xl border border-border-color shadow-sm">
                ...
                <button type="submit" class="enterprise-btn-primary">Enregistrer &rarr;</button>
            </div>
        </form>
    </div>
</x-app-layout>
